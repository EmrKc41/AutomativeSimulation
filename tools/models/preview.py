...
TAMIR = ("tamir", 100, 28)
# Üç hat; `src/factory.ts` HATLAR ile aynı. Bitmiş ürün alanı hepsinde x=140.
HATLAR = [("LINE-01", 0), ("LINE-02", 46), ("LINE-03", 92)]
BITMIS = 140
...
VARLIK_OLCEK = 0.35  # ASSET_SCALE — konumlarla aynı; modeller gerçek metrede
MERKEZ_X = 82.5
HAT_Y = 8
...
def hat_kur(klasor, hat_y):
    """
    Bir üretim hattı: konveyör, istasyonlar, tamir hücresi, doli koridoru,
    bitmiş ürün kapısı ve hattaki araçlar.

    Üç hat da birbirinin kopyası; fark yalnızca plan üzerindeki Y'si ve
    ürettiği model.
    """
    # Konveyör istasyonların **arasında** akıyor, içinden değil.
    for plan_x in range(34, 128, 5):
        if any(abs(plan_x - ist[1]) < 5 for ist in ISTASYONLAR):
            continue
        yerlestir(klasor, "konveyor", plan_x, hat_y)

    for ad, plan_x, _sablon_y, donme in ISTASYONLAR:
        plan_y = hat_y
        if ad == "weld":
            # Kaynak hücresi: konveyörün iki yanında robot
            yerlestir(klasor, "robot", plan_x, plan_y + 3.2, math.radians(180))
            yerlestir(klasor, "robot", plan_x, plan_y - 3.2, 0)
            yerlestir(klasor, "operator", plan_x + 4, plan_y + 4.2, math.radians(180))
        else:
            yerlestir(klasor, ad, plan_x, plan_y, donme)
            if ad == "pres":
                yerlestir(klasor, "pres-koc", plan_x, plan_y)
            if ad == "boyahane":
                yerlestir(klasor, "robot", plan_x - 1.5, plan_y + 2.0, math.radians(180))
            if ad == "montaj":
                yerlestir(klasor, "robot", plan_x - 3.5, plan_y - 3.1, 0)
                yerlestir(klasor, "operator", plan_x + 3.5, plan_y + 3.5, math.radians(180))
            if ad == "kalite":
                yerlestir(klasor, "operator", plan_x + 3.4, plan_y + 3.5, math.radians(180))

    # Tamir hücresi ve tamircisi — hattın dışında, kendi alanında.
    yerlestir(klasor, TAMIR[0], TAMIR[1], hat_y + TAMIR[2])
    yerlestir(klasor, "operator", TAMIR[1] - 1.5, hat_y + TAMIR[2] + 2.6, math.radians(180))

    # Bitmiş ürünün salondan çıktığı kapı ve park eden araçlar.
    yerlestir(klasor, "gecis", BITMIS - 10, hat_y)
    for dx in (0, 5):
        yerlestir(klasor, "arac", BITMIS + dx, hat_y)

    # Doli koridoru: depo hizasından hattın sonuna, oradan her hücreye sapma.
    koridor_y = hat_y + 18
    guzergah(DEPO[0] - 2, koridor_y, 124, koridor_y, 1.2)
    for hedef_x in (40, 60, 80, 100, 120):
        guzergah(hedef_x, koridor_y, hedef_x, hat_y + 10, 1.2)
    ...
    # Bant yüksekliği modelde 0.68 m; konveyör gerçek boyuta 1,25 kat
    # büyütüldüğü için o çarpanı da taşıyor.
    bant = 0.68 * 1.25 * VARLIK_OLCEK
    for _, plan_x, _sablon_y, _ in ISTASYONLAR:
        yerlestir(klasor, "arac", plan_x, hat_y, 0, yukseklik=bant)
    yerlestir(klasor, "arac", TAMIR[1], hat_y + TAMIR[2])


def sahne_kur(klasor):
    bpy.ops.wm.read_factory_settings(use_empty=True)

    # Zemin — komuta merkezindeki grafit ile aynı aile.
    bpy.ops.mesh.primitive_plane_add(size=260, location=(0, 0, -0.01))
    zemin = bpy.context.active_object
    mat = bpy.data.materials.new("zemin")
    mat.use_nodes = True
    bsdf = mat.node_tree.nodes["Principled BSDF"]
    bsdf.inputs["Base Color"].default_value = (0.030, 0.033, 0.040, 1)
    bsdf.inputs["Roughness"].default_value = 0.95
    zemin.data.materials.append(mat)

    # Üç üretim hattı; her biri kendi modelini üretiyor.
    for _hat_ad, hat_y in HATLAR:
        hat_kur(klasor, hat_y)

    # Mal kabul — bağımsız alan, en solda. Tır düz yanaşıyor, çapraz değil.
    yerlestir(klasor, "guvenlik", RAMPA[0] - 20, RAMPA[1] + 22, math.radians(90))
    yerlestir(klasor, "rampa", RAMPA[0], RAMPA[1])
    yerlestir(klasor, "tir", RAMPA[0] - 15, RAMPA[1])
    yerlestir(klasor, "forklift", RAMPA[0] - 8, RAMPA[1] - 4)
    ...
    # İç lojistik deposu — üç hattı da besleyen ortak raflar
    for dy in (-4, 0, 4):
        yerlestir(klasor, "raf", DEPO[0], DEPO[1] + dy, math.radians(90))

    # Sevkiyat — her hattın kendi yükleme şeridi, ortak yol ve tek çıkış kapısı.
    yerlestir(klasor, "sevkiyat", SEVKIYAT[0] - 6, SEVKIYAT[1], math.radians(180))
    for _hat_ad, hat_y in HATLAR:
        guzergah(SEVKIYAT[0], hat_y, 190, hat_y, 3.0)
        yerlestir(klasor, "oto-tasiyici", SEVKIYAT[0] + 4, hat_y)
    guzergah(190, HATLAR[0][1], 190, HATLAR[-1][1], 3.0)
    guzergah(190, HATLAR[1][1], 210, HATLAR[1][1], 3.0)
    yerlestir(klasor, "guvenlik", 204, HATLAR[1][1])
...
def kamera_kur():
    ...
    # Kamera tesisin **ortasına** nişan alıyor.
    #
    # Tesis üç hat derin ve mal kabulden çıkış kapısına ~110 birim geniş; plan
    # merkezinin dünya karşılığı yaklaşık (-3, 16). Önceki sürüm sabit bir
    # noktaya bakıyordu ve kadrajın alt yarısı boş zemin çıkıyordu.
    hedef_x, hedef_y = -3.0, 16.0
    mesafe, yukseklik_acisi = 95.0, math.radians(34)
    bpy.ops.object.camera_add(
        location=(
            hedef_x,
            hedef_y - mesafe * math.cos(yukseklik_acisi),
            mesafe * math.sin(yukseklik_acisi),
        )
    )
    kamera = bpy.context.active_object
    kamera.rotation_euler = (math.radians(90) - yukseklik_acisi, 0, 0)
    kamera.data.lens = 30
...
def render(cikti):
    ...
    sahne.render.resolution_x = 1600
    sahne.render.resolution_y = 820
